...
final class MoviesViewModel {
    ...
    func load(reset: Bool = true) async {
        ...
        do {
            ...
            errorMessage = "加载电影列表失败: \(error.localizedDescription)"
            canLoadMore = false
            print(errorMessage ?? "加载电影列表失败")
        }
    }
    ...
    func loadMoreIfNeeded(currentItem: MovieDTO? = nil) async {
        ...
        do {
            ...
            errorMessage = "加载更多电影失败: \(error.localizedDescription)"
            print(errorMessage ?? "加载更多电影失败")
        }
    }
